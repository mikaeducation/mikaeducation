...
document.addEventListener("DOMContentLoaded", function () {
    ...
    inputs.forEach((input) => {
        ...
        input.addEventListener("drop", function (e) {
            ...
            answerDiv.className =
                "js-dropped-answer px-2 py-1 m-1 rounded border";
    ...
    document.addEventListener("quiz-submitted", function (e) {
        ...
        if (result.data?.details) {
            Object.entries(result.data.details).forEach(([group, items]) => {
                ...
                // Apply feedback
                items.forEach((item) => {
                    const droppedEl = Array.from(
                        groupBox.querySelectorAll(".js-dropped-answer")
                    ).find((el) => el.textContent.trim() === item.answer);

                    if (!droppedEl) return;

                    if (item.correct) {
                        droppedEl.classList.add(
                            "border-green-500",
                            "bg-green-100"
                        );
                    } else {
