                        ...
                        <div class="space-y-4 sm:border-2 sm:border-bluee3 rounded">
                            ...
                            <div class="w-full space-y-4 sm:pl-2 pb-2 ">
                                <div class="w-full text-blue31">
                                    <form id="formEvaluation8_2_1" action="{{ route('asessment-evaluasi-submit') }}" method="POST" class="space-y-4 sm:pr-4 text-justify">
                                        @csrf
                                        <input type="hidden" name="asessment_id" value="3">
                                        <input type="hidden" name="after_asessment_id" value="2">
                                        <div id="user-answers" data-answers="{{ json_encode(session('user_answers')) }}"></div>
                                        @foreach(range(21, 30) as $qid)
                                            <input type="hidden" name="question_{{ $qid }}" value="{{ session('user_answers')['question_' . $qid] ?? '' }}">
                                        @endforeach
                                        <div class="space-y-2">
                                            <p class="font-medium text-xl">A. Akses dan Kemudahan Belajar</p>
                                            @foreach($questions as $index => $question)
                                                @if($question->question_id >= 31 && $question->question_id <= 35)
                                                    <div>
                                                        ...
                                                        <div class="flex items-start justify-start space-x-3 mb-1">
                                                                <input 
                                                                    ...
                                                                    {{ (session('user_answers')['question_' . $question->question_id] ?? '') == $score ? 'checked' : '' }}    
                                                                >
                                        ...
                                        <div class="space-y-4 mt-8">
                                            ...
                                            @foreach($questions as $index => $question)
                                                @if($question->question_id >= 36 && $question->question_id <= 40)
                                                    <div>
                                                        <div class="mb-2 flex gap-2">
                                                            <p class="asessmen1-num font-medium">{{ $index + 1 }}.</p>
                                                            <p class="asessmen1-quest">{{ $question->question_text }}</p>
                                        ...
                                        <div class="space-y-4 mt-8">
                                            ...
                                            @foreach($questions as $index => $question)
                                                @if($question->question_id >= 41 && $question->question_id <= 45)
                                                    <div>
                                                        <div class="mb-2 flex gap-2">
                                                            <p class="asessmen1-num font-medium">{{ $index + 1 }}.</p>
                                                            <p class="asessmen1-quest">{{ $question->question_text }}</p>
